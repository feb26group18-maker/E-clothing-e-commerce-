import "./EditSellerProfile.css"; import { useEffect, useState } from "react"; import { useNavigate, useParams } from "react-router-dom"; export default function SellerEditProfile(){ const {id} = useParams(); const navigate = useNavigate(); const [seller,setSeller] = useState({ name:"", email:"", mobile:"", shopName:"", gstNumber:"", businessAddress:"" }); const [msg,setMsg] = useState(""); useEffect(()=>{ const token = localStorage.getItem("token"); console.log("TOKEN:",token); console.log("SELLER ID:",id); fetch(`http://localhost:8081/users/seller/profile/${id}`,{ method:"GET", headers:{ "Authorization":`Bearer ${token}`, "Content-Type":"application/json" } }) .then((res)=>{ console.log("STATUS:",res.status); if(!res.ok){ throw new Error("Failed to fetch seller"); } return res.json(); }) .then((data)=>{ console.log("SELLER DATA:",data); setSeller(data); }) .catch((err)=>{ console.log(err); }); },[id]); const handleChange=(e)=>{ setSeller({ ...seller, [e.target.name]:e.target.value }); }; const handleUpdate = () => { const token = localStorage.getItem("token"); fetch(`http://localhost:8081/users/seller/${id}`, { method:"PUT", headers:{ "Authorization": `Bearer ${token}`, "Content-Type":"application/json" }, body: JSON.stringify({ name:seller.name, mobile:seller.mobile, shopName:seller.shopName, gstNumber:seller.gstNumber, businessAddress:seller.businessAddress }) }) .then((res) => { if(!res.ok){ throw new Error("Profile update failed"); } return res.text(); }) .then((data)=>{ console.log("Update Response:", data); setMsg("Profile updated successfully ✅"); setTimeout(()=>{ navigate(`/seller/profile`); },1500); }) .catch((err)=>{ console.log(err); setMsg(err.message); }); }; return( <div className="edit-profile-container">
